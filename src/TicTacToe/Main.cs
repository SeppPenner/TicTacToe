...
public partial class TicTacToeForm : Form
{
    ...
    private int TranslateMouseClickToPosition(int x, int y)
    {
        ...
        var pixelsPerColumn = this.ticTacToePanel.Width / TicTacToeGame.Columns;
        var pixelsPerRow = this.ticTacToePanel.Height / TicTacToeGame.Rows;
        var row = y / pixelsPerRow;
        var col = x / pixelsPerColumn;
        var position = (row * TicTacToeGame.Columns) + col;
        return position;
    }
    ...
    private void DrawPieces(Graphics graphics)
    {
        ...
        for (var i = 0; i < TicTacToeGame.Rows * TicTacToeGame.Columns; i++)
        {
    ...
    private void DrawGrid(Graphics graphics)
    {
        ...
        float pixelsPerRow = this.ticTacToePanel.Width / TicTacToeGame.Rows;
        float pixelsPerColumn = this.ticTacToePanel.Height / TicTacToeGame.Columns;

        // Now draw the grid lines
        var p = new Pen(Brushes.Black) { Width = 6 };

        for (var i = 0; i < TicTacToeGame.Rows - 1; i++)
        {
            graphics.DrawLine(p, pixelsPerColumn * (i + 1), 0, pixelsPerColumn * (i + 1), this.ticTacToePanel.Height);
        }

        // Now draw the horizontal lines
        for (var j = 0; j < TicTacToeGame.Columns - 1; j++)
        {
    ...
    private PointF GetPieceDrawingCoordsFromPosition(int position)
    {
        ...
        var row = position / TicTacToeGame.Columns;
        var column = position % TicTacToeGame.Columns;
        var pixelsPerRow = this.ticTacToePanel.Height / TicTacToeGame.Rows;
        var pixelsPerColumn = this.ticTacToePanel.Width / TicTacToeGame.Columns;
